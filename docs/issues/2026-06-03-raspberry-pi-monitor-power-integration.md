...
`ASAMIRU_DISPLAY_ENABLED` の既定値は false とし、明示的に有効化された場合だけ DRM/DDC 監視を起動する。DDC/CI が利用不能な場合はエラーを表示し、暗黙に無効化しない。

### サーバー責務

`apps/api` に `DisplayService` を設け、ハードウェアアクセスを一箇所に集約する。
...
ハードウェア制御 API は loopback 接続だけを許可し、LAN に無認証で公開しない。双方向連動の SSE も kiosk Chromium だけが購読できるようにする。
...
asamiru のスリープ・起床遷移から、モニターの待機・復帰を制御する。状態取得は設定画面とデバッグ用途に限定する。
...
モニター側の手動 ON/OFF をユーザー操作として扱う双方向連動を追加する。アプリ自身のコマンドによる遷移との関連付け、連続観測による debounce、起床時間帯でも有効な手動スリープ状態を実装する。
...
DRM hotplug event と DDC/CI `D6` のポーリング結果を同じ時系列で確認するため、`apps/api/scripts/check-display-power.ts` を追加した。

通常の観測モードはモニターへ制御コマンドを送らない。スクリプトを起動したまま、モニターの物理ボタンで OFF/ON してログを確認する。

```sh
pnpm --filter api check:display-power -- --display 1
```
...
```sh
# 1回だけ現在状態を取得
pnpm --filter api check:display-power -- --display 1 --once

# 特定 connector だけ表示
pnpm --filter api check:display-power -- --display 1 --connector card1-HDMI-A-1

# D6=05 で消灯し、確認後に D6=01 で復帰を試みる制御テスト
pnpm --filter api check:display-power -- --display 1 --control-test
```
...
```sh
pnpm --filter api check:display-power -- --bus 10
```
...
`DisplayPower` enum と provider の対応表を入れる。型は `on | standby | suspend | off | unknown` の5値だが、`ddc-ci`（`D6`）は実機上 `x01→on` / `x04→off` / それ以外→`unknown` の3値しか生成しない（check スクリプトの実装どおり）。`standby`/`suspend` は将来の CEC 等向けの予約値であることを明記し、provider ごとに「生成しうる値の部分集合」を表で定義する。受け手（Web）が来ない値に分岐を書かずに済む。
...
`DisplayDriver` 抽象を切ってテスト可能にする。check スクリプトは exec を内包しているが、本番 `DisplayService` は `readConnection() / readPower() / setStandby() / setPowerOn()` を持つ driver interface を定義し、`DdcCiDriver` / `CecDriver` / `NoneDriver` と、ハードウェア無しでロジックを検証する `FakeDriver` を用意する。oscillation 抑止やステートマシンはこの層で純粋にユニットテストできる（WSL/CI でも回せる）。AGENTS の「過度なフォールバック禁止」に沿い、provider が `none` 以外で driver が利用不能なら例外を上げて設定画面に出す（暗黙の `none` 降格はしない）。
