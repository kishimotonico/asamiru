...
スクレイピング（年1回・手動）はテスト対象外とし、「毎日使う画面に関わるロジック・API・仕様」に絞ってテスト基盤を導入し、優先度順にテストを追加する。

## テスト基盤

フレームワークは vitest（vite エコシステムと整合）。コンポーネントの描画テストは行わず、純粋関数・パーサー・統合（fetchモック）に絞るため `@testing-library` は導入しない。
...
共通課題: テスト対象の純粋関数の多くが**非 export**。テストのため対象関数に `export` を付与する（大規模な純粋モジュール切り出しはリグレッションリスクを避けて行わない。export 追加は最小限の妥協とする）。

## テスト対象（優先度順）

### フェーズ1: 発車案内の中核（今回の変更の中心）
...
- `pnpm -r test` で全テストが緑になること
- `pnpm build`（`tsc -b` 含む）がテストファイルを拾わず通ること
- フェーズごとにコミット（フェーズ1→2→3）。各フェーズ完了時に該当テストが通ることを確認
